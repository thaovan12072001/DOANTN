...
if (isset($_COOKIE["user"])) {
  ...
  if ($permission == 1) {
    ...
    foreach (selectAll("SELECT * FROM donhang WHERE MONTH(STR_TO_DATE(thoigian, '%d-%m-%Y')) = $currentMonth AND YEAR(STR_TO_DATE(thoigian, '%d-%m-%Y')) = $currentYear") as $item3) {
      $tongtienthang += $item3['tongtien'];
    }
?>
    <!-- partial -->
      ...
      <div class="content-wrapper">
        <div class="row">
          ...
          <div class="col-sm-4 grid-margin">
            <div class="card">
              <div class="card-body">
                <h5 class="addfont">Doanh Thu Tháng <?= $currentMonth ?></h5>
                <div class="row">
                  <div class="col-8 col-sm-12 col-xl-8 my-auto">
                    <div class="d-flex d-sm-block d-md-flex align-items-center">
                      <h2 class="mb-0"><?= number_format($tongtienthang) ?>đ</h2>
                      <!-- <p class="text-danger ml-2 mb-0 font-weight-medium">-2.1% </p> -->
                    </div>
                    <!-- <h6 class="text-muted font-weight-normal"> 9.61% Since last month</h6> -->
                  </div>
                  <div class="col-4 col-sm-12 col-xl-4 text-center text-xl-right">
                    <i class="icon-lg mdi mdi-wallet-travel text-warning ml-auto"></i>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div class="col-12 grid-margin">
            <div class="card">
              <div class="card-body">
                <h4 class="card-title addfont">Top 5 Sản Phẩm Bán Chạy Tháng <?= $currentMonth ?></h4>
                  ...
                  <table class="table">
                    <thead>
                      <tr>
                        ...
                        <th class="addfont" style="width: 400px">Số Lượng Đơn Hàng</th>
                        <th class="addfont" style="width: 400px">Tổng Doanh Thu</th>
                      </tr>
                    </thead>
                    <tbody>
                      ...
                      <?php
                      $stt = 1;
                     foreach (selectAll("SELECT sanpham.id, sanpham.gia,  sanpham.ten, SUM(ctdonhang.soluong) AS tong_so_luong
                     FROM donhang
                     INNER JOIN ctdonhang ON donhang.id = ctdonhang.id_donhang
                     INNER JOIN sanpham ON ctdonhang.id_sanpham = sanpham.id
                     WHERE MONTH(STR_TO_DATE(thoigian, '%d-%m-%Y')) = $currentMonth AND YEAR(STR_TO_DATE(thoigian, '%d-%m-%Y')) = $currentYear
                     ...
                     LIMIT 5") as $rows) {
                      ?>
                          <tr class="addfont">
                            ...
                            <td>
                            <?= number_format($rows['tong_so_luong'] * $rows['gia']) ?>   đ            
                            </td>
                          </tr>
                      <?php
                     }
